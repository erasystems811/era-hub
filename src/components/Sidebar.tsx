...
const NAV: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: '/comms/billing',    label: 'Billing & Usage', icon: <CreditCard className="w-4 h-4" />, accent: 'pink' },
    ...
    return (
      ...
      >
        {({ isActive: active }) => (
          ...
          >
            {active && (
              <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full" style={{ background: accent.bar }} />
            )}
              ...
              >
                {({ isActive: active }) => (
                  <div
                    ...
                    style={active ? {
                      background: childAccent.bg,
                      color: childAccent.text,
                    } : {
                      color: 'rgba(255,255,255,0.45)',
                    }}
                  >
                    {active && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 rounded-r-full" style={{ background: childAccent.bar }} />
                    )}
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        {/* Coming soon */}
        <div
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm mt-2 cursor-not-allowed select-none"
          style={{ color: 'rgba(255,255,255,0.22)' }}
        >
          <Zap className="w-4 h-4" />
          <span>ERA Connect</span>
          <span
